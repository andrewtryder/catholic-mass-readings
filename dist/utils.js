import { DATE_FMT, NEW_TESTAMENT_BOOKS, OLD_TESTAMENT_BOOKS, } from "./constants.js";
...
export function getBookFromVerse(link, text) {
...
export function getReadingNumber(text) {
...
export function stripBookAbbreviationsFromText(text) {
    return text.replace(ABBREVIATED_BOOK_PATTERN, "").trim();
}
function getBookNameFromLink(link) {
    const match = BOOK_LINK_PATTERN.exec(link);
    return match ? match[1] : null;
}
function* getBookAbbreviationsFromText(text) {
    for (const match of text.matchAll(ABBREVIATED_BOOK_PATTERN)) {
        yield match[1];
    }
}
export function lookupBook(key) {
    if (!key)
        return null;
